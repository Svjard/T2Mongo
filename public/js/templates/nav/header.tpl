  <!-- Brand and toggle get grouped for better mobile display -->
  <a id="leftmenu-trigger" class="tooltips" data-toggle="tooltip" data-placement="right" title="" data-original-title="Toggle Sidebar"></a>
  <div class="navbar-header pull-left">
    <a class="navbar-brand" href="/">
      <img src="/images/teradata-logo.png" style="position: absolute;height: 40px;top: 28px;">
      <span style="display: inline-block;margin-left: 165px;width: 200px;margin-top: 3px;">T2Mongo Demo</span>
    </a>
  </div>
  <ul class="nav navbar-nav pull-right toolbar">
    <li style="border-left: 1px solid #27292d;">
      <a href="#/home" style="padding-top: 12px !important;padding-bottom: 12px !important;">
        <span><i class="fa fa-home"></i></span>
      </a>
    </li>
  </ul>
